...
- **Rule 5 — Add clarification when the selected task diverges from the user's request**: Whenever the task you create differs from what the user explicitly asked for (e.g., upgrading to a different version, choosing a higher-level task type, or skipping a requested change due to compatibility), you MUST add a clarification question to `clarifications.json` explaining what was selected, why it differs, and asking the user to confirm or adjust. Never silently override the user's intent.