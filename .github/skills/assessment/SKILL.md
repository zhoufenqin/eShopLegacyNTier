...
1. **Check Project Type and Prerequisites**:
   - **For Java projects**: Check MCP tool availability in this order:
     1. **Primary**: Check if 'appmod-run-assessment-action' MCP tool is available
        ...
        - **Before calling this tool**, read `{workspace-path}/.github/modernize/assessment/reports/assessment-config.yaml` and check the `assessmentDomains` field. If it contains `security`, remove it from the list. If `security` is the only domain (i.e., the list becomes empty after removal), set `assessmentDomains` to an empty array `[]`. Write the modified config back to the same file before proceeding. This prevents duplicate security assessments since security is already handled separately.
        - **When calling this tool, do NOT pass any configuration parameters** (no config, no domains). The tool internally resolves the assessment config from the file you just modified.
