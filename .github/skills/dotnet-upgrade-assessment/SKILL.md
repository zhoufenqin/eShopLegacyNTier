...
- `assessment_dir` (required): Base assessment output directory (e.g., `.github/modernize/assessment`). The skill will locate the versioned report directory under `{assessment_dir}/reports/`.
...
```
{assessment_dir}/reports/report-{reportId}/
```

Where `reportId` is a 14-digit timestamp in `yyyyMMddHHmmss` format (e.g., `20250514065424`). Find the latest one:

```bash
REPORT_DIR=$(ls -d {assessment_dir}/reports/report-[0-9]* 2>/dev/null | sort | tail -1)
```
