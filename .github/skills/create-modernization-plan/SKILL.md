...
3. **Clarification** (only when the `ask_user` tool is available): If there are any open issues or ambiguities that need user input:
...
5. **Generate plan and tasks**: Generate plan.md and tasks.json using the appropriate templates:
    ...
    **Clarification Outcomes in Plan**: Incorporate all clarification answers from steps 3–4 into `plan.md` and `tasks.json`:
    ...
    - Record all clarification questions and their outcomes in the **"## Open Questions"** section of `plan.md`:
      - Answered: `- [x] Q: ... → A: ...`
      - Unanswered/skipped: `- [ ] ...`
...
6. **Playbook Compliance Validation** (only when playbook attachments are present):
...
1. All clarification questions have been asked via `ask_user`, a summary was confirmed by the user (or skipped), and answers are incorporated into `plan.md` and `tasks.json` with outcomes recorded in the "## Open Questions" section of `plan.md`
